...
import { Archivo, MontoValues, Presupuesto, SubidaArchivos } from '../../../interfaces/proyecto.interfaces';
...
@Component({
  selector: 'app-presupuesto-tab-estudio',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './presupuesto-tab-estudio.component.html',
  styleUrls: ['./presupuesto-tab-estudio.component.scss'],
  schemas: [CUSTOM_ELEMENTS_SCHEMA]
})
export class PresupuestoTabEstudioComponent implements OnInit {
  ...
  porcentajeIncentivoTotal: string = '';
  ...
  */
  actualizarMonto(porcentaje: string, tipo: string) {
    console.log("porcentaje:", porcentaje, "tipo:", typeof porcentaje);

    if(porcentaje === '') {
      porcentaje = "0";
    }
    let porcentajeNumerico = parseFloat(porcentaje);
    console.log("porcentajeNumerico:", porcentajeNumerico, "tipo:", typeof porcentajeNumerico);
    
    if(porcentajeNumerico > 100) {
      porcentajeNumerico = 100.00;
      this.porcentajeIncentivoTotal = porcentajeNumerico.toFixed(2);
    }

    if(porcentajeNumerico < 0) {
      porcentajeNumerico = 0.00;
      this.porcentajeIncentivoTotal = porcentajeNumerico.toFixed(2);
    }
        
    let totalIncentivos = parseCLP(this.totalIncentivos);    
    let incentivoTotal = totalIncentivos*porcentajeNumerico/100;
    let aportePropio = totalIncentivos - incentivoTotal;

    console.log("totalIncentivos:", totalIncentivos, "tipo:", typeof totalIncentivos);
    console.log("incentivoTotal:", incentivoTotal, "tipo:", typeof incentivoTotal);
    console.log("aportePropio:", aportePropio, "tipo:", typeof aportePropio);
    
    
    this.incentivoTotal = this.formatearCLP(incentivoTotal.toFixed(0));
    this.aportePropio = this.formatearCLP(aportePropio.toFixed(0));
    // this.calcularIncentivoTotal();
    this.calcularMontoPorcentaje(porcentajeNumerico, totalIncentivos);
  }


  private parseMontoValues(): MontoValues {
    return {
      aportePropio: parseCLP(this.aportePropio),
      totalIncentivos: parseCLP(this.totalIncentivos),
      incentivoTotal: parseCLP(this.incentivoTotal)
    };
  }

  actualizarPorcentaje(monto: string, tipo: string) {
    let { aportePropio, totalIncentivos, incentivoTotal } = this.parseMontoValues();

    // Ensure values don't exceed total incentives
    if (incentivoTotal > totalIncentivos) {
      incentivoTotal = totalIncentivos;
    }
    if (aportePropio > totalIncentivos) {
      aportePropio = totalIncentivos;
    }

    // Handle type-specific updates
    switch (tipo) {
      case 'totalIncentivos':
        aportePropio = totalIncentivos;
        incentivoTotal = 0;
        break;
      case 'incentivoTotal':
        aportePropio = totalIncentivos - incentivoTotal;
        break;
      case 'aportePropio':
        incentivoTotal = totalIncentivos - aportePropio;
        break;
    }

    if (incentivoTotal > (totalIncentivos * 0.99)) {
      this.mensajeAlerta = 'El total de incentivos solicitado supera el 99% del del costo total de ejecución de la solicitud de pre-inversión';
    } else {
      this.mensajeAlerta = '';
    }

    // Update component properties
    const porcentaje = this.calcularPorcentajeMonto(incentivoTotal, totalIncentivos);
    this.porcentajeIncentivoTotal = porcentaje.toFixed(2);
    this.incentivoTotal = this.formatearCLP(incentivoTotal.toString());
    this.aportePropio = this.formatearCLP(aportePropio.toString());
  }
  ...
  */
  calcularIncentivoTotal() {
    ...
    const totalIncentivos = parseCLP(this.incentivoTotal) + aportePropio;
    // this.totalIncentivos = this.formatearCLP(totalIncentivos.toString());
    // this.costoTotalProyecto = this.totalIncentivos;
    
    // Validar si se excede el tope de incentivos
    const costo = aportePropio+totalIncentivos;
    // const solicitado = totalIncentivos;
    if (totalIncentivos > (costo * 0.99)) {
      this.mensajeAlerta = 'El total de incentivos solicitado supera el 99% del del costo total de ejecución de la solicitud de pre-inversión';
    } else {
      this.mensajeAlerta = '';
    }
  ...
  /**
  * Valida si se excedió el tope de ejecución.
  */
